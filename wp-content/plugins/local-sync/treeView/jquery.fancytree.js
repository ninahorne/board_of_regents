...
//		return this.parent.children.indexOf(this);
	...
	getDeSelectedNodes: function(stopOnParents) {
		var nodeList = [];
		this.visit(function(node){
			// if(!node.selected && (node.partsel == undefined || node.partsel === false)) {
			if(!node.selected && (node.partsel == undefined || node.partsel === false) ) {
				nodeList.push(node);
				if( stopOnParents === true ){
					return "skip"; // stop processing this branch
   ...
   */
	...
	*/
	...
	generateFormElements: function(selected, active, opts) {
		...
		if(selected === false){
			nodeList = this.getDeSelectedNodes(stopOnParents);
			$.each(nodeList, function(idx, node){
				$result.append($("<input>", {
					type: "checkbox",
					name: selectedName,
...
//            window.console.log("getNodeByKey(" + key + "): ", node.key);
	...
	getDeSelectedNodes: function(stopOnParents) {
		return this.rootNode.getDeSelectedNodes(stopOnParents);
	},
...
//				this._callHook("nodeSetExpanded", ctx, true);
				if(ctx.originalEvent.toElement.outerText && ctx.originalEvent.toElement.outerText.indexOf('Exclude') !== -1 || ctx.originalEvent.toElement.outerText && ctx.originalEvent.toElement.outerText.indexOf('Include') !== -1){
					return ;
				}
				this._callHook("nodeToggleExpanded", ctx);
			}
...
//				parent.ul.appendChild(node.li);
	...
	nodeRenderTitle: function(ctx, title) {
		...
		ares.push(nodeTitle);
		if(typeof node.data != 'undefined' && typeof node.data.size != 'undefined' && node.folder !== true) {
			ares.push("<span role='button' class='fancytree-size-key'> "+node.data.size+"</span>");
		}
...
//			this._fixSelectionState(node);
	...
	treeLoad: function(ctx, source) {
		if (typeof local_sync_fancy_tree_time != 'undefined' && local_sync_fancy_tree_time + 10 > jQuery.now() ) {
			if (local_sync_fancy_tree_request === ctx.options.source.data.action) {
				console.log('LS skip multiple request', local_sync_fancy_tree_request);
				return ;
			}
		}
		local_sync_fancy_tree_request = ctx.options.source.data.action;
		local_sync_fancy_tree_time = jQuery.now();
