...
const institutionsRefinements = document.querySelector(
  '#institutionsRefinements'
);
...
const subjectAreaRefinements = document.querySelector(
  '#subjectAreaRefinements'
);
...
function activateRefinementsLabels() {
  const refinementsLabels = document.querySelectorAll(
    '[data-refinementslabel]'
  );
...
function controlDisplayOfRefinementsDropdowns(e) {
  ...
  refinementContainers.forEach((container) => {
...
function initializeAlgolia() {
             ...
             hits.length
               ? hits
                   ...
                   <div class="results__info first">
                       ...
                       ${
                         item.satellite_campus &&
                         item.satellite_campus != 'none'
                           ? `<p class="results__satellite">${item.satellite_campus}</p>`
                           : ''
                       }
                   </div>
  ...
  const renderCurrentRefinements = (renderOptions, isFirstRender) => {
    ...
    widgetParams.container.innerHTML = `
               ...
               ${
                 getQueryParamsObjectFromString(window.location.search).query !=
                 undefined
                   ? `<li>${
                       getQueryParamsObjectFromString(window.location.search)
                         .query
                     }
  ...
  const renderPagination = (renderOptions, isFirstRender) => {
    const { currentRefinement, isLastPage, widgetParams, refine, nbPages } =
      renderOptions;
    ...
    document.querySelector('#pagination').innerHTML = `
               ...
               ${
                 page != 1
                   ? `
                  <li>
                  <a style="font-size:11px" href="" data-value="${1}"/>First</a></li>
               ...
               ${
                 !isLastPage
                   ...
                   <li>
                   <a style="font-size:11px" href="" data-value="${nbPages - 1}"/>Last</a></li>
                       `
                   : ''
               }`;
...
function setInitialStateFromQueryParams() {
  ...
  if (institution) {
    ...
    valArray.forEach((facet) => {
      courses_search.helper.addDisjunctiveFacetRefinement('institution', facet);
    });
  }
  ...
  if (modality) {
    ...
    valArray.forEach((facet) => {
      courses_search.helper.addDisjunctiveFacetRefinement('modality', facet);
    });
  }
  if (courseSubject) {
    const valArray = courseSubject.split(';');
    const formattedArray = valArray.map((item) =>
      decodeURI(item).replace(/%26/g, '&')
    );
  ...
  if (semester) {
    ...
    formattedArray.forEach((facet) => {
      courses_search.helper.addDisjunctiveFacetRefinement('semester', facet);
    });
  }
...
function setQueryParams(state) {
  ...
  const aroundLatLng = state.aroundLatLng;
  if (aroundLatLng) {
  ...
  facets.forEach((facet) => {
    ...
    if (refinements.length) {
      queryParamString = `${queryParamString}${facet}=${encodeURI(
        refinements.join(';').replace(/&/g, '%26')
      )}&`;
    }
  });
...
function getCurrentLocation(e) {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    () => {
      alert(
        'You have disabled location services for this website. Please change the settings in your browser to use this feature.'
      );
    }
  );
}
...
function generateQRCode() {
  ...
  paragraph.innerText =
    "This QR code can be downloaded and shared to bring people directly to these search results from their cellphone's camera.";
